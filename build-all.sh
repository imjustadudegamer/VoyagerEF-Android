...
echo "== [1/5] syncing canonical engine sources -> jni/efcode =="
...
mkdir -p "$ASSETS/vm"
cp -v "$HERE/ui.qvm"               "$ASSETS/vm/ui.qvm"
...
cp -v "$HERE/EFAndroid/app/build/outputs/apk/debug/app-debug.apk" \
      "$HERE/VoyagerEF-VULKAN-test.apk"

echo "DONE: $HERE/VoyagerEF-VULKAN-test.apk"
