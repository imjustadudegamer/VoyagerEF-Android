 ...
 * is present in the app's OBB directory before the engine starts.
 *
 * On Android 11+ ordinary file managers cannot write into
 * Android/obb/com.voyager.ef, so users cannot copy the paks there by hand.
 * This activity lets them pick the four files with the system document
 * picker (Storage Access Framework) and stream-copies them into the OBB
 * directory, which the app itself may always write to without permissions.
 *
...
public class ImportActivity extends Activity {
    ...
    private String missingPaksList() {
    ...
    private void showImportDialog(String extraLine) {
        String msg = (extraLine != null ? extraLine + "\n\n" : "")
                ...
                + "    pak0.pk3\n    pak1.pk3\n    pak2.pk3\n    pak3.pk3\n\n"
                + "(from the BaseEF folder of an installed copy; pak0.pk3 is "
                + "about 540 MB).\n\n"
                + "Copy them onto this device, then tap \"Select pak files\" "
                + "and pick all four.\n\n"
                + "Still missing: " + missingPaksList();

        AlertDialog.Builder b = new AlertDialog.Builder(this);
        b.setTitle("Game data required");
        b.setMessage(msg);
        b.setCancelable(false);
        b.setPositiveButton("Select pak files", new DialogInterface.OnClickListener() {
            @Override
