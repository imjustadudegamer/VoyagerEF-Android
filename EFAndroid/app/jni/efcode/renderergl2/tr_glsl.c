...
static void GLSL_GetShaderHeader( GLenum shaderType, const GLchar *extra, char *dest, int size )
{
	...
	if(glRefConfig.glslMajorVersion > 1 || (glRefConfig.glslMajorVersion == 1 && glRefConfig.glslMinorVersion >= 30))
	{
		...
		{
			// Use highp float for BOTH stages on GLES. mediump (fp16 on Mali)
			// snaps world coords to a coarse grid, collapsing/flipping triangles
			// as the MVP changes (flickering black triangles when panning). It must
			// match across stages or varyings fail to link, so highp everywhere.
	...
	else
	{
		if (qglesMajorVersion >= 2)
		{
			...
			// highp for both stages (fp16 collapses world geometry on pan; must
			// match across stages for varyings to link)
			Q_strcat(dest, size, "precision highp float;\n");
...
// --- Mali shader pre-warm: tiled GPUs defer GPU-ISA compilation to the first
// DRAW that uses each program, causing 100-350ms hitches mid-gameplay as panning
// brings new surface/light/material combinations into view. We collect every
// program here and force-compile them all with a tiny offscreen draw at load
// (GLSL_WarmupShaders), so there are no in-game stalls. ---
static GLuint s_warmPrograms[ 1024 ];
static int    s_numWarmPrograms = 0;
...
void GLSL_FinishGPUShader(shaderProgram_t *program)
{
	...
	if ( program->program && s_numWarmPrograms < (int)ARRAY_LEN( s_warmPrograms ) )
		s_warmPrograms[ s_numWarmPrograms++ ] = program->program;
}

// Force every linked program through an actual draw so the GL driver compiles
// it now (at load) rather than on first in-game use. Output is fully suppressed
// (1x1 scissor + color/depth masks off) so nothing is rendered.
static void GLSL_WarmupShaders( void )
{
	static const float tri[9] = { -1.f,-1.f,0.f,  1.f,-1.f,0.f,  0.f,1.f,0.f };
	GLuint vao = 0, vbo = 0;
	int i, startTime, endTime;

	if ( s_numWarmPrograms == 0 )
		return;
	...
	qglBufferData( GL_ARRAY_BUFFER, sizeof( tri ), tri, GL_STATIC_DRAW );
	qglEnableVertexAttribArray( ATTR_INDEX_POSITION );
	qglVertexAttribPointer( ATTR_INDEX_POSITION, 3, GL_FLOAT, GL_FALSE, 0, 0 );

	for ( i = 0; i < s_numWarmPrograms; i++ )
	{
		qglUseProgram( s_warmPrograms[i] );
		qglDrawArrays( GL_TRIANGLES, 0, 3 );
	}
	qglFinish();   // make the driver actually compile now, not defer
	...
	// invalidate cached GL state so the backend re-sets everything next frame
	GL_BindNullProgram();
	glState.currentVao = NULL;
	glState.vertexAttribsEnabled = 0;

	endTime = ri.Milliseconds();
	ri.Printf( PRINT_ALL, "Pre-warmed %i GPU shaders in %5.2f seconds\n",
		s_numWarmPrograms, ( endTime - startTime ) / 1000.0 );
}
...
void GLSL_InitGPUShaders(void)
{
	...
	// Compile all programs now (offscreen) to avoid first-use stalls during play.
	GLSL_WarmupShaders();
}
