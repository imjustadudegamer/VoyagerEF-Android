	...
	const char *label, float cr, float cg, float cb )
{
	...
	if ( label && *label ) {
		...
		float lx   = ( ncx + nhw ) * W - cap - strlen( label ) * size;  // right-anchored
		float ly   = ( ncy + nhh ) * H - size * 1.15f;                  // bottom-anchored
		const char *s = label;
		re.SetColor( black );
		while ( *s ) {
			int ch = *s & 255;
...
void CL_DrawTouchControls( void ) {
	...
	if ( !touchControl.moveActive && !touchControl.actionPressed
...
void SCR_DrawScreenField( stereoFrame_t stereoFrame ) {
	...
	// IDENTICAL to IN_TouchControlsActive in sdl_input.c.
